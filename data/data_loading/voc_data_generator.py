...
class VOCDataGenerator(Dataset):

    def __init__(self, to_fit, images_dir, label_txt, image_shape=(448, 448, 3), grid_size=7, max_bb_count=2, num_classes=20):
        self.to_fit = to_fit
    ...
    def __getitem__(self, idx):
        ...
        for b in range(n):
            ...
            cv2.rectangle(img_show, pt1=pt1, pt2=pt2,
                          color=(0, 255, 0), thickness=1)
        cv2.imwrite(os.path.join(
            debug_dir, 'test_{}.jpg'.format(idx)), img_show)

        h, w, _ = img.shape
        # normalize (x1, y1, x2, y2) w.r.t. image width/height.
        boxes /= torch.Tensor([[w, h, w, h]]).expand_as(boxes)
        target = self.encode(boxes, labels)  # [S, S, 5 x B + C]

        img = cv2.resize(img, dsize=(
            self.image_size, self.image_size), interpolation=cv2.INTER_LINEAR)
        # assuming the model is pretrained with RGB images.
    ...
    def encode(self, boxes, labels):
        ...
        # width and height for each box, [n, 2]
        boxes_wh = boxes[:, 2:] - boxes[:, :2]
        boxes_xy = (boxes[:, 2:] + boxes[:, :2]) / \
            2.0  # center x & y for each box, [n, 2]
        for b in range(boxes.size(0)):
            xy, wh, label = boxes_xy[b], boxes_wh[b], int(labels[b])

            ij = (xy / cell_size).ceil() - 1.0
            # y & x index which represents its location on the grid.
    ...
    def random_scale(self, img, boxes):
        ...
        h, w, _ = img.shape
        img = cv2.resize(img, dsize=(int(w * scale), h),
                         interpolation=cv2.INTER_LINEAR)

        scale_tensor = torch.FloatTensor(
            [[scale, 1.0, scale, 1.0]]).expand_as(boxes)
    ...
    def random_shift(self, img, boxes, labels):
        ...
        center = center + \
            torch.FloatTensor([[dx, dy]]).expand_as(center)  # [n, 2]
        mask_x = (center[:, 0] >= 0) & (center[:, 0] < w)  # [n,]
        mask_y = (center[:, 1] >= 0) & (center[:, 1] < h)  # [n,]
        # [n, 1], mask for the boxes within the image after shift.
        ...
        shift = torch.FloatTensor(
            [[dx, dy, dx, dy]]).expand_as(boxes_out)  # [m, 4]
    ...
    def random_crop(self, img, boxes, labels):
        ...
        center = center - \
            torch.FloatTensor([[x, y]]).expand_as(center)  # [n, 2]
        mask_x = (center[:, 0] >= 0) & (center[:, 0] < w)  # [n,]
        mask_y = (center[:, 1] >= 0) & (center[:, 1] < h)  # [n,]
        ...
        if len(boxes_out) == 0:
            return img, boxes, labels
        shift = torch.FloatTensor(
            [[x, y, x, y]]).expand_as(boxes_out)  # [m, 4]
...
def test():
    ...
    images_dir = 'data/VOC_allimgs/'
    label_txt = ['data/voc2007.txt', 'data/voc2012.txt']

    dataset = VOCDataset(True, images_dir, label_txt)
